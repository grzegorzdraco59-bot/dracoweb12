...
try
{
    ...
    var sampleCommand = new MySqlCommand("SELECT * FROM zamowieniahala_v LIMIT 2", connection);
    using var sampleReader = await sampleCommand.ExecuteReaderAsync();
    
    if (sampleReader.HasRows)
    {
        Console.WriteLine("\n=== Przykładowe rekordy (pierwsze 2) ===");
        Console.WriteLine("-".PadRight(100, '-'));
        
        int rowCount = 0;
        while (await sampleReader.ReadAsync() && rowCount < 2)
        {
            Console.WriteLine($"\nRekord {rowCount + 1}:");
